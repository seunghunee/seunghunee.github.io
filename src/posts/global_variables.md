...
`static`은 "static(고정된, 정적인)"이라는 영단어에서 왔는데, `static`으로 선언하면 _정적 변수_(_Static variable_)가 된다. 보통 프로그램이 실행되는 동안 **동적으로** 할당되었다가 해제되었다가 하는 일반 변수와 다르게, 정적변수는 컴파일될 때 프로그램안에 **미리 정적으로 할당**된다. 그리고 정적 **변수**라는 이름에서도 알 수 있듯이 상수와는 다르게 선언할 때 `mut` 키워드를 사용할 수 있고 값을 변경할 수 있다. 다만 그냥 값을 단순한 대입문으로 변경해보면 안전하지 않다고 컴파일러가 에러를 낸다.
...
물론, 에러 메시지에 따라서 `unsafe` 함수나 `unsafe` 블록을 사용하면 값을 쉽게 변경할 수 있다.

```rust
static mut PI: f64 = 3.141592;
unsafe { PI = 1.0 }
```

하지만, 그냥 이렇게 끝내기에는 뭔가 찝찝하다.
분명 뭔가 이유가 있기 때문에 컴파일러는 정적 변수의 값을 단순한 대입문으로 변경하는 것이 안전하지 않다고 우리에게 주의를 주고 있을 것이다.
그 이유가 무엇이고 어떻게 해결하는지는 조금 뒤에 알아보도록 하고, 지금은 상수와 정적 변수에 마저 주목해 보자.
중요한 이 둘의 차이점중 하나는 바로 컴파일 결과물에 값이 저장되는 방법이 다르다는 것이다.

상수는 컴파일되면 **상수가 사용되는 모든 곳에 상수의 값이 곧바로 삽입**된다.
...
따라서 `let`이나 `static`으로 선언된 변수처럼 프로그램이 실행될 때 메모리의 특정 영역을 차지하는 것이 아니다.

반면 정적 변수의 값은 컴파일되면 **특정 영역에 저장**되고 프로그램이 실행되면 고유의 메모리 주소도 가지며 정적 변수가 사용되는 곳에서는 이 주소의 값을 참조한다.
다만 일반적으로 프로그램 실행 도중 영역의 크기가 변하는 스택(Stack)영역이나 힙(Heap)영역과는 다르게,
정적 변수의 값이 저장되는 [영역](https://en.wikipedia.org/wiki/Data_segment)은 컴파일될 때 저장공간의 크기와 장소가 결정되고 변하지 않는다.
즉, 하나의 정적 변수가 차지하는 메모리 공간은 프로그램이 실행되어 끝날 때까지 변하지 않으며 한 곳에 "**고정되어(static)**" 쓰인다.
따라서 프로그램이 끝날 때까지 존재하기 때문에 정적 변수는 `Drop`으로 소멸자(Destructor)를 구현해도 호출되지 않는다.

이렇게 값이 저장되는 방법의 차이를 알고 있으면 상황에 따라 더욱 효율적인 선택을 할 수 있다.
예를 들어 `const`으로 선언된 값의 크기가 매우 크고 아주 많은 곳에 삽입된다면 컴파일 결과물의 크기를 비효율적으로 크게 만들 수 있는데,
이럴 때 `const`대신 `static`을 사용해서 값은 한 곳에 저장해 두고 필요한 곳에서 그 값을 읽도록 할 수 있다.
